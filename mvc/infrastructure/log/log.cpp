...
std::atomic log_thread_id_enabled = true;
...
void SetLogThreadIdEnabled(bool enabled) {
  log_thread_id_enabled.store(enabled, std::memory_order_relaxed);
}
...
bool GetThreadIdEnabled() {
  return log_thread_id_enabled.load();
}
...
LogStream::~LogStream() {
  ...
  if (GetThreadIdEnabled()) {
    output << "[" << std::this_thread::get_id() << "] ";
  }

  output << GetLogLevelColor(level_) << "[" << GetLogLevelName(level_) << "]" << BashColors::RESET;
...
const char* LogStream::GetLogLevelColor(LogLevel level) {
  switch (level) {
    case LogLevel::TRACE:
      return BashColors::CYAN;
    case LogLevel::DEBUG:
      return BashColors::BLUE;
    case LogLevel::INFO:
      return BashColors::GREEN;
    case LogLevel::WARNING:
      return BashColors::YELLOW;
    case LogLevel::ERROR:
      return BashColors::RED;
    case LogLevel::CRITICAL:
      return BashColors::MAGENTA;
    default:
      return BashColors::RESET;
  }
}
